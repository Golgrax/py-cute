...
def inventory_management_page():
    ...
    with doc.add(div(_class='mobile-view', style="max-width:600px;")):
        with div(_class='content-wrapper'):
            ...
            # Form for Add/Update Item (simplified, no action handling here)
            with form(method="post", action="#", style="margin-bottom:20px; padding:15px; border:1px solid #ccc; background-color:white; border-radius:5px;"):
                # ... (form fields as before) ...
                div(label("ITEM ID:", fr="item_id"), input_(type="text", name="item_id", id="item_id"), _class="form-group")
                div(label("ITEM NAME:", fr="item_name"), input_(type="text", name="item_name", id="item_name"), _class="form-group")
                div(label("QUANTITY:", fr="item_quantity"), input_(type="number", name="quantity", id="item_quantity", min="0"), _class="form-group")
                div(label("PRICE:", fr="item_price"), input_(type="number", name="price", id="item_price", min="0", step="0.01"), _class="form-group")
                div(
