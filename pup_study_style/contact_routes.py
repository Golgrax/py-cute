    ...
    with doc.add(div(_class='mobile-view')) as mobile_view_div:
        with mobile_view_div.add(div(_class='content-wrapper')):
            ...
            with form(method='post', style="border:1px solid #ccc; padding:20px; border-radius:8px; background-color:white;"):
                div(label("Name:", fr="contact_name"), input_(type="text", name="name", id="contact_name", required=True), _class="form-group")
                div(label("Email Address:", fr="contact_email"), input_(type="email", name="email", id="contact_email", required=True), _class="form-group")
                div(label("Message:", fr="contact_message"), textarea(name="message", id="contact_message", rows="5", required=True), _class="form-group")
